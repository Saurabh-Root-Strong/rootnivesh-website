...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    try {
        ...
        // Stop-loss: same shape as targets â€” a free list, first number is the
        ...
        // UPSERT â€” don't create a duplicate row when the team re-pastes the SAME
        // running call to advance its progress (1st target â†’ 2nd target â†’ â€¦).
        ...
        // Call date â€” when the call was actually given. Blank = now.
        ...
        $vals = [
            ...
            ':is_public'    => 1,   // every posted call publishes â€” no hidden-by-accident trap
        ...
        if ($existingId) {
            ...
            $flash = 'Same call advanced (#' . $existingId . ') â€” updated existing row, no duplicate.';
        } else {
            // No OPEN call to advance â†’ this would be a NEW row. Authoritative,
            ...
            // identical ALREADY-CLOSED call exists â€” same symbol+side, entry within
            ...
            if ($dupId) {
                ...
                $flash = 'âš ï¸ Already recorded (#' . $dupId . ', ' . $res . ' on ' . $when . ') â€” '
                       . 'same entry, targets and stop-loss. NOT posted again to avoid a duplicate. '
                       . 'If it really is a fresh trade, change a price or use â€œPost anywayâ€.';
            } else {
                $pdo->prepare(
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_status') {
    try {
        ...
        if ($prog === 'running') {
            ...
            // Nth target achieved but the call is still running â†’ stays in Live.
...
/* Order-insensitive numeric SET key â€” "535, 550, 562" and "562,550,535" and
...
function progress_select_html($c) {
    ...
    $html .= '<option value="running"' . $sel('running') . '>Still running â€” no target hit</option>';
    // Intermediate targets keep the call LIVE (only "All" closes it).
    for ($k = 1; $k < $N; $k++) {
        $html .= '<option value="live_' . $k . '"' . $sel('live_' . $k) . '>'
               . ordinal($k) . ' Target Achieved â€” still live</option>';
    }
    $html .= '<option value="all"' . $sel('all') . '>âœ… All ' . ($N > 0 ? $N . ' ' : '') . 'Targets Achieved â†’ Performance</option>';
    $html .= '<option value="stop"' . $sel('stop') . '>ðŸ›‘ Stop-loss Hit â†’ Performance</option>';
    $html .= '<option value="closed"' . $sel('closed') . '>â˜‘ï¸ Closed manually â†’ Performance</option>';
    $html .= '<option value="cancelled"' . $sel('cancelled') . '>âœ– Cancelled (hide)</option>';
    $html .= '</select>';
    return $html;
}
...
function build_wa_message($c) {
    $arrow = $c['action'] === 'BUY' ? 'ðŸŸ¢' : 'ðŸ”´';
    $msg  = $arrow . ' *' . strtoupper($c['call_type']) . " CALL*\n\n";
    $msg .= '*' . $c['action'] . '*: ' . $c['symbol'];
    if (!empty($c['company_name'])) $msg .= ' (' . $c['company_name'] . ')';
    $msg .= "\n";
    $msg .= 'Entry: â‚¹' . number_format(floatval($c['entry_price']), 2) . "\n";
    if (!empty($c['targets']))            $msg .= 'Targets: ' . $c['targets'] . "\n";
    elseif ($c['target_price'] !== null)  $msg .= 'Target: â‚¹' . number_format(floatval($c['target_price']), 2) . "\n";
    if (!empty($c['stop_losses']))        $msg .= 'Stop-Loss: ' . $c['stop_losses'] . "\n";
    elseif ($c['stop_loss'] !== null)     $msg .= 'Stop-Loss: â‚¹' . number_format(floatval($c['stop_loss']), 2) . "\n";
    if (!empty($c['thesis']))        $msg .= "\nThesis: " . $c['thesis'] . "\n";
    $msg .= "\nâ€” RootNivesh Research\nSEBI Reg. No. " . SEBI_REG;
...
<title>RootNivesh Admin â€” Calls</title>
...
<div class="admin-layout">
  ...
  <main class="admin-main">
    ...
    <section class="admin-card">
      ...
      <p style="color:#8A9BB0; font-size:13px; margin:0 0 10px">
        ...
        <em>â€œBUY RELIANCE Entry 2450 Target 2500 SL 2420â€</em>.
      </p>
      <textarea id="pasteBox" rows="4" class="admin-paste"
                placeholder="BUY RELIANCE&#10;Entry 2450&#10;Target 2500&#10;SL 2420"></textarea>
      <div style="margin-top:8px; display:flex; gap:8px; align-items:center; flex-wrap:wrap">
        <button type="button" class="admin-btn" onclick="parsePaste()">â¤“ Parse into form</button>
    ...
    <section class="admin-card">
      ...
      <form id="addCallForm" method="post" class="admin-form">
        ...
        <div class="admin-row">
          ...
          <label>Call date â€” blank = today
            <input type="date" name="posted_at">
          </label>
        </div>
        <div class="admin-row">
          <label>Targets (T1, T2, T3â€¦)
        ...
        <div class="admin-row">
          <label>Where does this call go? *
            <select name="post_status" id="postStatus" onchange="togglePostExit()">
              ...
              <option value="running">ðŸ†• Fresh Call â†’ Calls page (live, nothing hit yet)</option>
              <option value="all">âœ… All Targets Achieved â†’ Performance (win)</option>
              <option value="stop">ðŸ›‘ Stop-loss Hit â†’ Performance (loss)</option>
              <option value="closed">â˜‘ï¸ Closed manually â†’ Performance</option>
            </select>
          </label>
          <label id="postExitWrap" style="display:none">Booked / avg exit â‚¹
        ...
        <label>Thesis
          <textarea name="thesis" rows="3" placeholder="Why this call â€” research thesis. Will be included in the WhatsApp share."></textarea>
        </label>
      ...
      ?>
        <div class="admin-call <?php echo $statusClass; ?>">
          <div class="admin-call-head">
            ...
            <?php if (!$c['is_public']): ?><span class="admin-call-status status-loss" title="Hidden from the public site">ðŸ”’ PRIVATE</span><?php endif; ?>
            <span class="admin-call-date"><?php echo date('d M Y, H:i', strtotime($c['posted_at'])); ?><?php if (!empty($c['created_by'])): ?> Â· by <?php echo htmlspecialchars($c['created_by']); ?><?php endif; ?></span>
          </div>
          <div class="admin-call-prices">
            Entry â‚¹<?php echo number_format($c['entry_price'], 2); ?>
            <?php if (!empty($c['targets'])): ?>Â· Targets <?php echo htmlspecialchars($c['targets']); ?><?php elseif ($c['target_price'] !== null): ?>Â· Target â‚¹<?php echo number_format($c['target_price'], 2); ?><?php endif; ?>
            <?php if (!empty($c['stop_losses'])): ?>Â· SL <?php echo htmlspecialchars($c['stop_losses']); ?><?php elseif ($c['stop_loss'] !== null): ?>Â· SL â‚¹<?php echo number_format($c['stop_loss'], 2); ?><?php endif; ?>
            <?php if (!empty($c['targets_hit']) && $c['status'] === 'open'): ?>Â· <strong style="color:#3FB950"><?php echo intval($c['targets_hit']); ?> target<?php echo intval($c['targets_hit']) > 1 ? 's' : ''; ?> hit</strong><?php endif; ?>
            <?php list($rr, $gain) = call_rr_gain($c); ?>
            <?php if ($rr   !== null): ?>Â· R:R 1:<?php echo number_format($rr, 2); ?><?php endif; ?>
            <?php if ($gain !== null): ?>Â· Tgt <?php echo ($gain >= 0 ? '+' : '') . number_format($gain, 2); ?>%<?php endif; ?>
            <?php if ($c['exit_price']   !== null): ?>Â· Exit â‚¹<?php echo number_format($c['exit_price'], 2); ?><?php endif; ?>
            <?php if ($c['pnl_pct']      !== null): ?>Â· PnL <strong><?php echo ($c['pnl_pct'] >= 0 ? '+' : '') . number_format($c['pnl_pct'], 2); ?>%</strong><?php endif; ?>
          </div>
          ...
          <div class="admin-call-actions">
            ...
            <?php if (!empty($c['shared_to_wa_at'])): ?>
              <span class="admin-call-shared">âœ“ Shared <?php echo date('d M, H:i', strtotime($c['shared_to_wa_at'])); ?></span>
            <?php endif; ?>
            ...
            <details style="display:inline-block; margin-left:8px">
              ...
              <form method="post" class="admin-inline-form">
                ...
                <input type="number" step="0.01" name="exit_price" placeholder="Exit â‚¹ (optional)" title="Optional â€” overrides the auto exit (final target for All Achieved, the stop for Stop-loss Hit). Use a blended average if you booked partials." value="<?php echo $c['exit_price'] !== null ? htmlspecialchars($c['exit_price']) : ''; ?>">
  ...
  <script>
    ...
    function parsePaste() {
      ...
      if (!raw) { msg.textContent = 'Nothing to parse â€” paste a message first.'; return; }
      ...
      const num  = (s) => { if (!s) return ''; const n = parseFloat(String(s).replace(/[,â‚¹\s]/g, '')); return isNaN(n) ? '' : n; };

      // Normalise so one grammar handles both labelled ("Entry -> â‚¹ 1006")
      // and inline ("BUY RELIANCE Entry 2450") styles: unify arrows -> ":",
      // strip markdown/quotes, collapse spaces but KEEP newlines.
      const text = raw
        .replace(/[â†’âž¤âžœâž”]/g, '->')   // â†’ âž¤ âžœ âž”
      ...
      // Stock / symbol â€” labelled only (company name, any case, may be multi-word).
      ...
      // Outcome detection â€” an "achieved/booked" message posts a winner directly.
      if (/\b(achiev|booked|profit\s*booked|target\s*hit|tgt\s*hit|all\s*targets?|sl\s*hit|stop\s*hit)\b/i.test(text)) {
        ...
        const exitM = text.match(/\b(?:booked|exit|exited|done|closed?)\s*(?:at|@|:|near)?\s*â‚¹?\s*(\d[\d,]*(?:\.\d+)?)/i);
        if (exitM) set('post_exit', num(exitM[1]));
        found.push(stopHit ? 'outcome:SL-hit' : 'outcome:achieved');
        togglePostExit();
      }

      // Thesis is left blank on parse â€” the team writes it themselves rather
      // than dumping the raw WhatsApp message into the field.

      msg.textContent = found.length
        ? 'âœ“ Filled: ' + found.join(', ') + '. Review the fields, then Post call.'
        : 'Could not auto-detect fields â€” fill the form manually.';
    ...
    function togglePostExit() {
      ...
      // "All Targets Achieved" â†’ the realized exit is the FINAL target (last
    ...
    // the team can post a call that has ALREADY hit its 1st/2nd/â€¦ target.
    ...
    function rebuildPostStatus() {
      ...
      // count â€” 3, 4, 5â€¦ â€” and with or without spaces after the commas.
      const nums = tIn.value.split(',').map(s => s.trim()).filter(s => /\d/.test(s)).length;
      const keep = sel.value;
      let html = '<option value="running">ðŸ†• Fresh Call â†’ Calls page (live, nothing hit yet)</option>';
      for (let k = 1; k < nums; k++) {
        html += `<option value="live_${k}">${ordinalJs(k)} Target Achieved â†’ stays on Live Calls</option>`;
      }
      html += '<option value="all">âœ… All ' + (nums > 0 ? nums + ' ' : '') + 'Targets Achieved â†’ Performance (win)</option>';
      html += '<option value="stop">ðŸ›‘ Stop-loss Hit â†’ Performance (loss)</option>';
      html += '<option value="closed">â˜‘ï¸ Closed manually â†’ Performance</option>';
      sel.innerHTML = html;
    ...
    /* ---- Duplicate-call guard ----
       ...
       entry/targets/SL) by mistake â€” that creates a duplicate winner on the
    ...
    (function () {
      ...
      form.addEventListener('submit', function (e) {
        ...
        // (A) DUPLICATE of an already-closed call â†’ warn, allow explicit override.
        ...
        if (dup) {
          ...
          const ok = confirm(
            'âš ï¸ Already recorded â€” looks like a DUPLICATE.\n\n' +
            sym + ' (' + act + ') â€” entry â‚¹' + entry + ', targets ' + (dup.targets || 'â€”') + '\n' +
          ...
          if (forceEl) forceEl.value = '1';          // user confirmed â†’ bypass server block
        } else {
          ...
          if (open) {
            const ok = confirm(
              'â„¹ï¸ A LIVE ' + act + ' call on ' + sym + ' is already running ' +
              '(entry â‚¹' + open.entry + ', targets ' + (open.targets || 'â€”') + ').\n\n' +
        ...
        // Passed the gates â†’ submitting. Disable the button so a double-click
        // can't fire a second identical POST. (Deferred so the form still submits.)
        const btn = form.querySelector('button[type="submit"]');
        if (btn) setTimeout(() => { btn.disabled = true; btn.textContent = 'Postingâ€¦'; }, 0);
      });
    })();
    ...
    function markShared(id) {
      ...
      // Don't block the WhatsApp window from opening â€” just send.
